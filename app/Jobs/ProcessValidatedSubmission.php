    ...
    public function handle()
    {
        foreach ($this->submission->entries as $entry) {
            ...
            foreach ($entry->schedules as $schedule) {
                $entry->business->addSchedule($schedule->start_hour, $schedule->end_hour, $schedule->sunday, $schedule->monday, $schedule->tuesday, $schedule->wednesday, $schedule->thrusday, $schedule->friday, $schedule->saturday, $schedule->type, $schedule->section_of_day, $schedule->by_appointment, $schedule->by_appointment_contacts);
            }
